...
def process_file_pair_full(file_pair, stations, min_nobs=7):
    ...
    cnc_total = TRCO2 + RA_CO2 - GPP_CO2
...
# --- Generate BG timeseries from observations if >= 2 sites ---
# Count valid observations per time
valid_count = np.sum(~np.isnan(ds['XCO2_obs'].values), axis=1)

# Initialize BG_obs
BG_obs = np.full((ds.sizes['time'], ds.sizes['pid']), np.nan)

# Only compute min for times with >=2 valid obs
mask = valid_count >= 2
min_obs = np.full(ds.sizes['time'], np.nan)
min_obs[mask] = np.nanmin(ds['XCO2_obs'].values[mask, :], axis=1)

# Broadcast min_obs across all sites
BG_obs[mask, :] = min_obs[mask, None]

# Add to dataset
ds['XCO2_mod_BG'] = (('time', 'pid'), BG_obs)
ds['XCO2_mod_BG'].attrs['comment'] = "BG is the lowest obs. (min 2 measurements)"
...
outfile = outpath + f"processed_EM27_ICONART_{t0}_{t1}_obsBG.nc"
...
def plot_hourly_month(ds, pid, year, month, outdir=outpath + "Obs_BG"):
    os.makedirs(outdir, exist_ok=True)
    ...
    plt.plot(times, mod + bg, 'r-', lw=1.5, label=f"MOD ({stats_dict['MOD']})")
